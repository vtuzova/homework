...
class CurrencyConverter(QMainWindow):
    ...
    def updateConvertBtnStatus(self):
        ...
        val2 = self.resultAmount.value()
        x = bool(val) ^ bool(val2)
        print('x=', x, 'val=', val, 'val2=', val2)
        self.convertBtn.setEnabled(x)
    ...
    def keyPressEvent(self, qKeyEvent):
        ...
        value = self.srcAmount.value()
        value2 = self.resultAmount.value()
        y = bool(value) ^ bool(value2)
        if y and qKeyEvent.key() == Qt.Key_Return: 
            self.onClick()
        else:
